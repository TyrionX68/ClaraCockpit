...
const ENABLE_SUPABASE_SYNC = import.meta.env.VITE_ENABLE_SUPABASE_SYNC === 'true';
...
const EigentuemerPortalEnhanced = () => {
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  const [currentUser, setCurrentUser] = useState(null);
  const [selectedObjekt, setSelectedObjekt] = useState(null);
  const [loginError, setLoginError] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [dataSource, setDataSource] = useState(USE_LOCAL_AUTH ? 'JSON' : 'Supabase');

  // Demo users data - JSON fallback
  const localUsers = [
  ...
  // Demo objects data - JSON fallback
  const localObjekte = {
    "e001": [
      { 
        id: "obj001", 
      ...
      if (USE_LOCAL_AUTH) {
        ...
        if (ownerId && ownerName && loginTime) {
          ...
          if (hoursDiff < 24) {
            ...
            const userObjekte = localObjekte[ownerId] || [];
        ...
        console.log('Supabase session check not implemented yet');
      }
    } catch (error) {
  ...
  const authenticateLocal = (username, password) => {
    ...
    if (user) {
      ...
      // Set default selected object
      const userObjekte = localObjekte[user.id] || [];
      if (userObjekte.length > 0) {
        setSelectedObjekt(userObjekte[0]);
      }
  ...
  const authenticateSupabase = async (username, password) => {
    // TODO: Implement Supabase authentication
    console.log('Supabase authentication not implemented yet');
    setLoginError('Supabase-Authentifizierung noch nicht verfügbar');
    return false;
  };
  ...
  const handleLogin = async (event) => {
    ...
    try {
      ...
      if (USE_LOCAL_AUTH) {
        success = authenticateLocal(username, password);
      } else {
        success = await authenticateSupabase(username, password);
      }
      ...
      setLoginError('Anmeldefehler: ' + error.message);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const calculatePortfolioKPIs = (userId) => {
    const userObjekte = localObjekte[userId] || [];
            ...
            <div className="mt-4 text-center">
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                ...
                <div className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
                  dataSource === 'JSON' ? 'bg-yellow-400' : 'bg-green-400'
                }`}></div>
                {dataSource} Modus
              </span>
            </div>
  ...
  const userObjekte = localObjekte[currentUser.id] || [];
...
export default EigentuemerPortalEnhanced;
